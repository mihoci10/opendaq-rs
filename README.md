...
The Makefile mirrors the cl-opendaq workflow: it handles cloning the pinned openDAQ source, building the native libraries into `bin/<platform>/`, and regenerating the generated Rust bindings.
...
Override `OPENDAQ_REF` in the Makefile for a different upstream version. The generator lives in `tools/generate_bindings.py` (with `tools/parse_bindings.py` shared verbatim with cl-opendaq); regenerate whenever the pinned openDAQ version changes.
...
- `examples/` — stand-alone example programs, ported from cl-opendaq
- `tools/` — the binding generator
- `tests/` — integration tests, ported from the cl-opendaq FiveAM suites
